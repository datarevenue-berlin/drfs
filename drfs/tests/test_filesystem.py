...
try:
    ...
    warn("azureblobfs not installed, skipping filesystem tests depending on " "it.")
    AzureBlobFileSystem = False


EXPECTED_METHODS = {"remove", "ls", "open", "exists", "mv", "makedirs", "rmdir"}
...
def _get_fs_tuples():
    ...
    if S3FileSystem:
        res.append(("s3://test-bucket/some_file.txt", S3FileSystem))
    if GCSFileSystem:
        res.extend(
            [
    ...
    if AzureBlobFileSystem:
        res.extend(
            [("abfs://test-account/test-container/some_file.txt", AzureBlobFileSystem)]
        )
    return res


@pytest.mark.skip(
    "Non-deterministic behaviour of moto. Sometimes this fails. "
...
def test_remote_path(s3):
    ...
    p.unlink()
    assert list(p.parent.iterdir()) == [RemotePath("s3://test-bucket/test2.txt")]
    assert not p.exists()
...
def test_filesystems(tmpdir, s3):
    ...
    assert list(map(norm_local, fs.ls(tmpdir))) == list(map(norm_s3, s3.ls(bucket)))

    assert list(map(norm_local, fs.glob(tmpdir.joinpath("*.txt")))) == list(
        map(norm_s3, s3.glob(bucket.joinpath("*.txt")))
    )
